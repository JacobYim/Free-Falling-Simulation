...
function calculate_paticles_to_add(mp, fp_m, fp_r, density, dt, g, T, window_width, window_height, num_bottom, num_top, num_right, num_left){
    ...
    num_right   += density * sqrt((2*k*T)/(fp_m)) * dt / 2 * (Math.exp(-1*(vwx/a)**2)/sqrt(Math.PI) +(vwx/a)*erfc((-1)*vwx/a)) * window_height;
    num_left    += density * sqrt((2*k*T)/(fp_m)) * dt / 2 * (Math.exp(-1*(vwx/a)**2)/sqrt(Math.PI) -(vwx/a)*erfc(vwx/a)) * window_height;
    num_bottom  += density * sqrt((2*k*T)/(fp_m)) * dt / 2 * (Math.exp(-1*(vwy/a)**2)/sqrt(Math.PI) +(vwy/a)*erfc((-1)*vwy/a)) * window_width;
    num_top     += density * sqrt((2*k*T)/(fp_m)) * dt / 2 * (Math.exp(-1*(vwy/a)**2)/sqrt(Math.PI) -(vwy/a)*erfc(vwy/a)) * window_width;
